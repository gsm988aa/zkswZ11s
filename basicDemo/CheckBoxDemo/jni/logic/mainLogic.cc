...
static void onProtocolDataUpdate(const SProtocolData &data) {
	...
	//向本机串口发送 FF 55 00 01 01 01   可控制Checkbox1 勾选
	//向本机串口发送 FF 55 00 01 01 00   可控制Checkbox1 非勾选

	if (data.value == 1) { //当value为1，将Checkbox设置为勾选状态
		mCheckbox1Ptr->setChecked(true);
	} else if (data.value == 0) { //当value为0，将Checkbox设置为非勾选状态
		mCheckbox1Ptr->setChecked(false);
	}
}
