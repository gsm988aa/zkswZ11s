...
static void procParse(const BYTE *pData, UINT len) {
	...
	case CMDID_TEST_VALUE:
		if (len < 11) {
			LOGE("长度不够");
			return;
		}
		sProtocolData.year = pData[5];
		sProtocolData.month = pData[6];
		sProtocolData.day = pData[7];
		sProtocolData.hour = pData[8];
		sProtocolData.min = pData[9];
		sProtocolData.sec = pData[10];
		break;
	}
